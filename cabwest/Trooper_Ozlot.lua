...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 18246, item2 = 12670, item3 = 12670})) then -- Legion Order (6 signed), Sabertooth 2x Kitten Canine
		...
		e.other:Faction(440,5); -- Cabilis Residents
		e.other:Faction(445,5); -- Scaled Mystics
		e.other:Faction(442,5); -- Crusaders of Greenmist
		e.other:Faction(444,5); -- Swift Tails
		e.other:QuestReward(e.self,0,0,0,0,18247,800); -- Legion Order (7 signed)
	elseif(item_lib.check_turn_in(e.trade, {item1 = 12670, item2 = 12670})) then -- 2x Sabertooth Kitten Canine
	...
	elseif(item_lib.check_turn_in(e.trade, {item1 = 18246, item2 = 12670})) then -- Legion Order and Sabertooth Kitten Canine
		e.self:Say("I told you that I need two sabertooth kitten canines and the legion order.");
		e.other:SummonItem(18246);-- Legion Order
		e.other:QuestReward(e.self,0,0,0,0,12670,0); -- Sabertooth Kitten Canine
	elseif(item_lib.check_turn_in(e.trade, {item1 = 18246})) then -- Legion Order
		e.self:Say("I told you that I need two sabertooth kitten canines and the legion order.");
