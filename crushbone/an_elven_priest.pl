...
sub EVENT_ITEM{
  ...
  if($itemcount{6315}==1 && $itemcount{1900}==1){
   	...
   	quest::summonitem(6305);
     	quest::ding();
	quest::say("As promised, here is your mace. Let it not fall into dark hands as its power is hidden until combat when it will reveal its true nature. Fare thee well!");
	quest::exp(100000);
