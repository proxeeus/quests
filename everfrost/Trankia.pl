...
sub EVENT_ITEM {
  ...
  if(($itemcount{1347} == 1) && ($itemcount{18797} == 1) && ($itemcount{18798} == 1)) {
    quest::say("Ah, Wulfthan, you are at last avenged. Thank you, $name - please take this kilt as a reward for services well done."); # Proxeeus - fixed the dialogue.
    quest::summonitem(2365);
     quest::ding(); quest::exp(500);
  }
}

# Quest edited and updated by Proxeeus
