...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 30272})) then -- Harness of Control
		...
		-- start gridid 68 (Kromriff fort path)
		eq.start(68);	-- Grid ported from P2002 on 23/08/2026
	elseif(item_lib.check_turn_in(e.trade, {item1 = 30274})) then -- Giants Harness of Control
		quest_chain = "Coldain"
		...
		e.other:Ding();
		e.other:AddEXP(20000);
		-- start gridid 69 (Coldain cave path)
		eq.start(69);	-- Grid ported from P2002 on 23/08/2026
	end
...
function event_waypoint_arrive(e)
	...
	elseif(e.wp == 12 and quest_chain == "Kromriff" and not arrived_second) then
		...
		if(eq.get_entity_list():IsMobSpawnedByNpcTypeID(118086)) then
			eq.attack_npc_type(118086);
			eq.signal(118090,2);
		end
