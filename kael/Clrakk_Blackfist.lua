...
local item_lib = require('items');
	...
	if (item_lib.check_turn_in(e.trade, {item1 = 25115})) then
		...
		e.other:Faction(406, -20); --coldain
		e.other:Faction(436, -20); --cov
		e.other:SummonItem(25054); --Mask of Malediction 
	elseif(item_lib.check_turn_in(e.trade, {item1 = 25114})) then
		e.self:Say("You truly are a [bringer of strife], "..e.other:GetName()..". Take this. I hope it will assist you. Use it in our war against the Coldain scum.");
		e.other:Faction(419, 10); --kromrif
		e.other:Faction(448, 10); --kromzek
		e.other:Faction(406, -20); --coldain
		e.other:Faction(436, -20); --cov
		e.other:SummonItem(25051); --Dark Spear of Venom
	end
