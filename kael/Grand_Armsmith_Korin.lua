...
function event_trade(e)
	...
	if (e.other:GetFaction(e.self) <= 3) then --Kindly or lower
		if (item_lib.check_turn_in(e.trade, {item1 = 25349})) then
			e.other:SummonItem(31520);
		elseif (item_lib.check_turn_in(e.trade, {item1 =  25433})) then	
			e.other:SummonItem(31524);
		elseif (item_lib.check_turn_in(e.trade, {item1 =  25370})) then	
			e.other:SummonItem(31521);
		elseif (item_lib.check_turn_in(e.trade, {item1 =  25391})) then	
			e.other:SummonItem(31523);
		elseif (item_lib.check_turn_in(e.trade, {item1 =  25384})) then	
			e.other:SummonItem(31522);
		elseif (item_lib.check_turn_in(e.trade, {item1 =  30507})) then	
			e.other:SummonItem(2610);
		elseif (item_lib.check_turn_in(e.trade, {item1 =  25194})) then	
			e.other:SummonItem(2611);
		elseif (item_lib.check_turn_in(e.trade, {item1 =  26025})) then	
