...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 25279}, 0)) then -- Voucher for toolset
		e.self:Say("Here you go " .. e.other:GetCleanName() .. ". I dunno why master Wenglawks trusts you with this but take it anyways and don't mess up.");
		e.self:Emote("smacks his fists together as he looks at you.");
		e.other:QuestReward(e.self,0,0,0,0,25104); -- Summon: Coldain Toolset
	elseif(item_lib.check_turn_in(e.trade, {item1 = 25280}, 0)) then -- Voucher for spear 
	...
	elseif(item_lib.check_turn_in(e.trade, {item1 = 25281}, 0)) then -- Mechanical Net Delivery
