...
function event_say(e)
	if(e.message:findi("hail")) then
		if(e.other:GetCharacterFactionLevel(429) >= 100) then
		...
		elseif(e.other:GetCharacterFactionLevel(429) >= 0) then
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 30501}, 0)) then -- Dain head
		if(e.other:GetFaction(e.self) < 8) then
			e.other:Faction(429,500); -- Faction: King Tormax
			e.other:Faction(448,500); -- Faction: Kromzek
			e.other:Faction(436,-250); -- Faction: Yelinak
			e.other:Faction(405,-250); -- Faction: Dain Frostreaver IV
	...
	elseif(item_lib.check_turn_in(e.trade, {item1 = 24984}, 0)) then -- Yelinak head
		if(e.other:GetFaction(e.self) < 8) then
			e.other:Faction(429,500); -- Faction: King Tormax
			e.other:Faction(448,500); -- Faction: Kromzek
			e.other:Faction(436,-250); -- Faction: Yelinak
...
function event_timer(e)
	if(e.timer == "help") then
		e.self:Shout("Those who raise arms against me will suffer my wrath!");
		help_tormax(e);
	end
end
