...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 28600,platinum = 200}, 0)) then
	...
	elseif(item_lib.check_turn_in(e.trade, {item1 = 20474}, 0)) then -- Receipt
	...
	elseif(item_lib.check_turn_in(e.trade, {item1 = 25130}, 0)) then -- Giant Icewurm Talisman
	...
	elseif(item_lib.check_turn_in(e.trade, {item1 = 25110}, 0)) then -- Giant Siren Scale Cloak
	...
	elseif(item_lib.check_turn_in(e.trade, {item1 = 29068, platinum = 100}, 0)) then
		e.self:Say("Here is the information your employer requested. Now be gone. I have more business to take care of.");
...
function event_combat(e)
	if(e.joined) then
		e.self:Say("You will not live to regret that!");
	end
end
