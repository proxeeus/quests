...
sub EVENT_HP {
  ...
  if ($hpevent == 80) {
    ...
    $entid1 = quest::spawn2(126265,0,0,-117.5,946,184,256); # rallos puppet
    $entid2 = quest::spawn2(126163,0,0,-127.5,946,184,256); # tunare puppet
    $entid3 = quest::spawn2(126153,0,0,-137,946,184,256); # innoruuk puppet
  ...
  if ($hpevent == 50) {
    ...
    $entid1 = quest::spawn2(126249,0,0,-167.5,858,179,67.2); # solusek puppet
    $entid2 = quest::spawn2(126163,0,0,-145,835,179,0); # tunare puppet
    $entid3 = quest::spawn2(126153,0,0,-110,835,179,0); # innoruuk puppet
    $entid4 = quest::spawn2(126291,0,0,-84.5,858,179,435.2); # erollisi puppet
    $mob1 = $entity_list->GetMobID($entid1);
    $mob2 = $entity_list->GetMobID($entid2);
    $mob3 = $entity_list->GetMobID($entid3);
