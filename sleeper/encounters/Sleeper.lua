...
function ControllerSignal(e)
	if ( e.signal == 1 ) then
		...
		-- Shouldn't be random imo, commenting out for now
		...
		local kerafyrm = eq.spawn2(KERAFYRM_TYPE, 0, 0, sleeper:GetX(), sleeper:GetY(), sleeper:GetZ(), sleeper:GetHeading()):CastToNPC();
		sleeper:Depop();
...
function event_encounter_load(e)
	...
	eq.register_npc_event("Sleeper", Event.waypoint_arrive, KERAFYRM_TYPE, WaypointArrive);
	eq.register_npc_event("Sleeper", Event.slay, KERAFYRM_TYPE, SleeperSlay);
end
