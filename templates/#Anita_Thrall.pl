...
quest::say("Your teleport spells have been scribed!");}
  elsif ($text=~/spells/i){
   quest::say("I can scribe [Circle] or [Portal] spells, Also [Temperance], [Lesser Summon Corpse], [Summon Corpse], [Conjure Corpse], [Tash] - But you need to bring me the spell");}
  elsif (($text=~/lesser summon corpse/i) | ($text=~/Tashan/i) | ($text=~/circle/i)){
  ...
  elsif ($text=~/Tashania/i){
   quest::say("That is the Coin of Tash quest, go and see  Romar Sunto in the Temple of Solusek Ro.");}
  elsif ($text=~/tash/i){
   quest::say("These include merchant bought; Tashan (level 2), Tashani (level 18),  Quested [Tashania] (level 41), and Tashanian (level 57), Howl of Tashan (level 61), Echo of Tashan (level 72)");
   quest::say("I also can scribe you the spells Wind of Tashani (55), and Wind of Tashanian (60).");
   quest::say("You can bring the spells to me one at a time, as you aquire the needed level.");}
...
## Tash spells ##################
## Check flags
 if (($itemcount{15676} == 1) && ($bot_spell_5 == 1) | ($bot_spell_5 == 2) | ($bot_spell_5 == 3) | ($bot_spell_5 == 4) | ($bot_spell_5 == 5) | ($bot_spell_5 == 6)){ 
    quest::say("Those spells are already scribed.");
...
## Circle Spells
    ...
    quest::say("The Moonstones must be given to me one at a time.");
    $client->Message(6,"Your failed to scribe your spells!");
