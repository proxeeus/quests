function event_spawn(e)
	if(eq.get_entity_list():IsMobSpawnedByNpcTypeID(115148)) then	-- ##Loremaster_Sarl
		eq.depop(115148);
	end
end
...
function event_say(e)
	...
	elseif((e.message:findi("name of the slain") or e.message:findi("name of a slain")) and e.self:GetGrid() == 22) then
		...
		e.self:CastToNPC():AssignWaypoints(0);	
	...
	elseif(e.message:findi("black") and e.self:GetX() == 751 and e.self:GetY() == -225) then
		...
		eq.spawn2(115148,32,0,e.self:GetX(),e.self:GetY(),e.self:GetZ(),e.self:GetHeading());	-- ##Loremaster_Sarl
		eq.depop_with_timer();
	end	
end
